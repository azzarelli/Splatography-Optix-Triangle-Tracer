...
class OptixTriangles(nn.Module):
    ...
    def forward(self, o, d, N, colors, verts, update_verts):
        ...
        colors_dc = colors                   # (G,3)

        out = torch.ones((H, W, 3), device=o.device, dtype=colors_dc.dtype)  # white bg

        valid = (g_idx >= 0) & (g_idx < colors_dc.shape[0])
        out[valid] = colors_dc[g_idx[valid]]
